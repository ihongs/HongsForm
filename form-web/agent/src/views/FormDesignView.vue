          ...
          <div class="card-body p-4">
            ...
            <div class="row g-3 mb-4">
              ...
              <div class="col-12">
                ...
                <textarea v-model.trim="form.script" class="form-control font-monospace" rows="6" placeholder="// 字段联动脚本
form.onChange('country', (f) => {
  if (f.value() === '中国') {
    form.field('state').hide().clear();
  }
});"></textarea>
                <div class="form-text">使用 JavaScript 编写字段联动逻辑，仅在浏览器端执行，安全可控</div>
              </div>
              <div class="col-12">
...
async function loadForm() {
  ...
  try {
    ...
    form.startAt = data.config?.startAt ? new Date(data.config.startAt) : null
    form.endAt = data.config?.endAt ? new Date(data.config.endAt) : null
