<template>
  <main class="container py-5">
    <div class="mx-auto" style="max-width: 720px;">
      ...
      <!-- 已提交提示 -->
      <div v-else-if="notStarted" class="card shadow-sm border-0">
        <div class="card-body text-center p-5">
          <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" fill="#f59e0b" class="mb-3" viewBox="0 0 24 24" stroke-width="0" stroke="currentColor">
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15l-5-5 1.41-1.41L11 14.17l7.59-7.59L20 8l-9 9z"/>
          </svg>
          <h1 class="h3 mb-3">表单尚未开放填报</h1>
          <p class="text-secondary mb-0">开始时间：{{ startTimeStr }}</p>
        </div>
      </div>

      <div v-else-if="expired" class="card shadow-sm border-0">
        <div class="card-body text-center p-5">
          ...
          <h1 class="h3 mb-3">表单已截止</h1>
          <p class="text-secondary mb-0">截止时间：{{ endTimeStr }}</p>
        </div>
      </div>
...
async function loadForm() {
  ...
  try {
    ...
    // 检查时间限制
    const now = Date.now()
    const startAt = data.config?.startAt ? new Date(data.config.startAt).getTime() : null
    const endAt = data.config?.endAt ? new Date(data.config.endAt).getTime() : null
    
    if (startAt && startAt > now) {
      notStarted.value = true
      startTimeStr.value = formatTime(data.config.startAt)
      return
    }
    
    if (endAt && endAt < now) {
      expired.value = true
      endTimeStr.value = formatTime(data.config.endAt)
      return
    }
