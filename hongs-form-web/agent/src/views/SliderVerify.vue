  ...
  <div class="hongs-captcha" :style="{ width: `${width}px` }">
    <div class="captcha-container" :style="{ height: `${height}px` }">
      ...
      <div v-if="status === 'fail'" class="captcha-overlay fail">
        <span class="icon">✗</span>
        <span class="text">Verification failed, please retry</span>
      </div>
    </div>
...
const emit = defineEmits(['ready', 'success', 'fail', 'refresh'])

const bgCanvas = ref(null)
const blockCanvas = ref(null)

const ordeal = ref(null)
...
function drawBlockHole(ctx, x, y, size) {
  ctx.save()
