...
```json
{
  "name": "表单名称",
  "title": "表单标题",
  "description": "表单描述，支持 markdown 格式",
  "fields": [],
  "config": {},
  "script": "附加脚本"
}
...
字段名应遵循以下规则：
- 建议按 `inputType` 自动生成，第一个字段名为 `inputType` 本身，如 `name`、`email`、`phone`
- 后续字段依次添加数字后缀：`name1`、`name2`、`phone1`、`phone2` 等
- 删除后自动补位，例如已有：`phone`、`phone2`，再新增时生成 `phone1`
...
- `name`：字段名，用于提交数据时的键名。
- `type`：数据类型，如 `string`、`number`、`integer`、`boolean`、`array`；缺省会按 inputType 推断。
- `inputType`：前端控件类型，见下方 inputType 类型。
- `title`：字段标题，用于数据列表、提交详情等场景展示。
- `description`：字段说明，显示在表单控件下方。
- `label`：可选的表单标签，未设置时表单内使用 `title`。
- `placeholder`：占位提示，显示在输入控件里面。
- `enum`：可选值列表。
...
- `required`：是否必填，`true`、`false`，缺省为 `false`。
...
| `select` | `type: "string", enum, labels` | 下拉单选 |
| `radio` | `type: "string", enum, labels` | 单选 |
| `check` | `type: "array", items.enum, labels` | 多选，必选时增加 `minItems: 1` |
